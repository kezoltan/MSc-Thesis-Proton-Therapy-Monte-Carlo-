...
def plot_kurtosis_variance_heatmap(L, kur1, var1, del1, del2, folder_path, log_kurt=False, level=None):
    ...
    axes[0].set_title("Estimated Kurtosis", fontsize=8)
    axes[0].set_xlabel(r"Depth $x$, cm", fontsize=8)
    axes[0].set_ylabel(r"$y$, cm", fontsize=8)
    ...
    axes[1].set_title(rf"Estimated Beam Axis Node Statistics ($y$={y_center:.2f})", fontsize=8)
    axes[1].set_xlabel(r"Depth $x$, cm", fontsize=8)
    axes[1].set_ylabel("Normalised Value", fontsize=8)
    ...
    fig.suptitle(rf"{title_seg} MLMC Test Statistics ($\ell={level}$, M={N_conv_test})", fontsize=10, x=title_center, ha="center")
...
def plot_alpha_beta_map(alpha, beta, folder_path):
    ...
    axes[0].set_title(r"$\alpha$", fontsize=10)
    axes[0].set_xlabel(r"Depth $x$, cm", fontsize=9)
    axes[0].set_ylabel(r"$y$, cm", fontsize=9)
    axes[0].set_aspect("equal")
    axes[0].tick_params(axis="both", which="major", labelsize=8)
    ...
    axes[1].set_title(r"$\beta$", fontsize=10)
    axes[1].set_xlabel(r"Depth $x$, cm", fontsize=9)
    axes[1].tick_params(axis="both", which="major", labelsize=8)
    #axes[1].set_ylabel("y")
    ...
    fig.suptitle(rf"{title_seg} MLMC Test: $\alpha$, $\beta$ Estimation (M={N_conv_test})", y=0.97, fontsize=10)
    ...
    ax_line.set_xlabel(r"Depth $x$, cm", fontsize=8)
    ax_line.set_ylabel("Estimated Value", fontsize=8)
    ax_line.set_title(rf"{title_seg} MLMC Test: Beam Axis Convergence Estimates (M={N_conv_test}, $y$={y_center})", fontsize=9)
    ...
    ax_line.tick_params("both", which="major", labelsize=8)
    plt.tight_layout()
